...
class MinStack {
    ...
    /*    Success:
        Runtime:5 ms, faster than 68.15% of Java online submissions.
        Memory Usage:41.7 MB, less than 26.45% of Java online submissions.*/
    class Node {
        int value;
        int min;
    ...
    public void push(int x) {
        if (stack.isEmpty()) stack.push(new Node(x,x));
            ...
            Node tail = stack.peek();
            if (tail.min > x) {
                stack.push(new Node(x,x));
            } else {
                stack.push(new Node(x,tail.min));
            }
        }
